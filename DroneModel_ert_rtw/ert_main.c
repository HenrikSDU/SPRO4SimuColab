...
void *baseRateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(DroneModel_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(DroneModel_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(DroneModel_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(DroneModel_M->extModeInfo, 2, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(DroneModel_M, true);
    }
  }
